...
// ----------------------------------------------------------
#include "R3BTofdCal2HistoS473.h"
...
R3BTofdCal2HistoS473::R3BTofdCal2HistoS473()
    : FairTask("R3BTofdCal2HistoS473", 1)
...
R3BTofdCal2HistoS473::R3BTofdCal2HistoS473(const char* name, Int_t iVerbose)
...
InitStatus R3BTofdCal2HistoS473::Init()
{
...
void R3BTofdCal2HistoS473::SetParContainers()
{
...
{
    uint64_t n1, n2;
	std::vector<Int_t> NSync = {0, 0, 0, 0, 0, 0};
};

void R3BTofdCal2HistoS473::Exec(Option_t* option)
{
	counter++;
	//cout << counter << endl;
	...
	*/
	...
	if(std::isnan(timeLos) || timeLos == 0.)
		return;
    ...
    for (auto it = bar_map.begin(); bar_map.end() != it; ++it)
    {
        ...
        for (; top_i < top_vec.size() && bot_i < bot_vec.size();)
        {
            ...
            // this way the los time will be at 0.
            auto top_ns =
                fmod(top->GetTimeLeading_ns() + c_range_ns, c_range_ns);
            auto bot_ns =
                fmod(bot->GetTimeLeading_ns() + c_range_ns, c_range_ns);
            /*
            ...
            if (std::abs(dt_mod) < c_bar_coincidence_ns)
            {
                ...
                if (fTofdQ < 0.1)
                {
					...
					else
                    {
                        ...
                        if (!par)
                        {
                            LOG(error) << "R3BTofdCal2HitS473::Exec : Hit par not found, Plane: " << iPlane
                                       << ", Bar: " << iBar;
                            continue;
					...
					if(THit < 0) NSync.at(0)++;
					if(THit > 0 && THit <= 500.) NSync.at(1)++;
				   	if(THit > 500. && THit < 1000.) NSync.at(2)++;
					if(THit > 1000. && THit < 1500.) NSync.at(3)++;
					if(THit > 1500. && THit < 2000.) NSync.at(4)++;
					if(THit > 2000. && THit < c_range_ns / 2) NSync.at(5)++;
					if(THit > c_range_ns / 2) cout << "THit out of bounds" << endl;
                ...
                else
                {
                    ...
                    // Time of hit
                    auto THit = (bot_ns + top_ns) / 2. - timeLos - para->GetSync();
					while(THit < -c_range_ns / 2)
							THit += c_range_ns;
					while(THit > c_range_ns / 2)
							THit -= c_range_ns;
                	...
                	if (!fTofdSmiley)
                	{
                    	if (NULL == fhTot1vsPos[iPlane - 1][iBar - 1])
                    	{
                        	...
                        	if (iPlane < 3)
                            	fhTot1vsPos[iPlane - 1][iBar - 1] = new TH2F(strName, "", 2000, -100, 100, 4000, 0., 200.);
                        	if (iPlane > 2)
                            	fhTot1vsPos[iPlane - 1][iBar - 1] = new TH2F(strName, "", 2000, -100, 100, 4000, 0., 200.);
                    	...
                    	if (NULL == fhTot2vsPos[iPlane - 1][iBar - 1])
                    	{
                        	...
                        	if (iPlane < 3)
                            	fhTot2vsPos[iPlane - 1][iBar - 1] = new TH2F(strName, "", 2000, -100, 100, 4000, 0., 200.);
                        	if (iPlane > 2)
                            	fhTot2vsPos[iPlane - 1][iBar - 1] = new TH2F(strName, "", 2000, -100, 100, 4000, 0., 200.);
...
void R3BTofdCal2HistoS473::CreateHistograms(Int_t iPlane, Int_t iBar)
{
    ...
    if (NULL == fhTsync[iPlane - 1])
    {
        ...
        fhTsync[iPlane - 1] = new TH2F(strName, strName2, 50, 0, 50, 10000, -2500, 2500.);
    ...
    if (NULL == fhSqrtQvsPosToT[iPlane - 1][iBar - 1])
    {
        ...
        fhSqrtQvsPosToT[iPlane - 1][iBar - 1] =
            new TH2F(strName, "", bx, -100, 100, by, 0., max_charge * 4);
...
void R3BTofdCal2HistoS473::FinishEvent() {}

void R3BTofdCal2HistoS473::FinishTask()
{
    for (Int_t i = 0; i < N_TOFD_HIT_PLANE_MAX; i++)
    {
        if (fh_tofd_TotPm[i])
	...
	cout << counter << " / " << maxx << " events used." << endl;

	cout << endl << endl << "TSync Values in" << endl;
	cout << "negative: " << NSync.at(0) << endl;
	cout << "[0., 500.]: " << NSync.at(1) << endl;
	cout << "[500.,1000.]: " << NSync.at(2) << endl;
	cout << "[1000.,1500.]: " << NSync.at(3) << endl;
	cout << "[1500.,2000.]: " << NSync.at(4) << endl;
	cout << "[2000.,2500.]: " << NSync.at(5) << endl;
}

Double_t R3BTofdCal2HistoS473::walk(Double_t Q, Double_t par1, Double_t par2, Double_t par3, Double_t par4, Double_t par5)
{
    Double_t y = 0;
...
Double_t R3BTofdCal2HistoS473::saturation(Double_t x)
{
    Double_t kor;
    Int_t voltage = 600;
    if (voltage == 600)
