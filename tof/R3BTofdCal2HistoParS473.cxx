...
#include "R3BTofdCal2HistoParS473.h"
...
R3BTofdCal2HistoParS473::R3BTofdCal2HistoParS473()
    : FairTask("R3BTofdCal2HistoParS473", 1)
    , fCalItemsLos(NULL)
    , fUpdateRate(1000000)
    , fMinStats(100000)
    , fTrigger(-1)
...
R3BTofdCal2HistoParS473::R3BTofdCal2HistoParS473(const char* name, Int_t iVerbose)
...
InitStatus R3BTofdCal2HistoParS473::Init()
{
...
void R3BTofdCal2HistoParS473::SetParContainers()
{
...
void R3BTofdCal2HistoParS473::Exec(Option_t* option) {}

void R3BTofdCal2HistoParS473::FinishEvent() {}

void R3BTofdCal2HistoParS473::FinishTask()
{
    if (fParameter == 1)
    {
    ...
    if (fParameter == 3)
    {
        ...
        if (fTofdSmiley)
        {
            ...
            Double_t para2[4];
            Double_t min2 = -35.; // -40 effective bar length
            Double_t max2 = 35.;  // 40 effective bar length = 80 cm
...
void R3BTofdCal2HistoParS473::calcOffset()
{
...
void R3BTofdCal2HistoParS473::calcToTOffset(Double_t totLow, Double_t totHigh)
{
...
void R3BTofdCal2HistoParS473::calcSync()
{
...
void R3BTofdCal2HistoParS473::calcVeff()
{
...
void R3BTofdCal2HistoParS473::calcLambda(Double_t totLow, Double_t totHigh)
{
...
void R3BTofdCal2HistoParS473::doubleExp(TH2F* histo, Double_t min, Double_t max, Double_t* para)
{
	...
	TString path = "/u/jsimon/installs/macros/analysis/figs/q23/DExp/" + name + ".png";
	cfit_exp->SaveAs(path);
    // gPad->WaitPrimitive();
    gSystem->Sleep(3000);
    delete gr1;
    delete gr2;
...
void R3BTofdCal2HistoParS473::smiley(TH2F* histo, Double_t min, Double_t max, Double_t* para)
{
	...
	TString histname = "/u/jsimon/installs/macros/analysis/figs/" + hname + ".pdf";
	cfit_smiley->SaveAs(histname);
    // gPad->WaitPrimitive();
    gSystem->Sleep(3000);
    delete histo1;
    delete histo2;
...
void R3BTofdCal2HistoParS473::zcorr(TH2F* histo, Int_t min, Int_t max, Double_t* pars)
{
    Double_t par[3000] = { 0 };
...
void R3BTofdCal2HistoParS473::setWalks()
{
	for(Int_t i = 0; i < 4; i++)
	{
...
ClassImp(R3BTofdCal2HistoParS473)
